...
exports.totalcase = () => {
   var file = fs.readFileSync("../Chucky.js").toString()
...
exports.buffergif = async (image) => {
  ...
  await sleepy(4000)
	...
	var buffer5  =  await  fs.readFileSync(`./UrusMedia/trash/${filename}.mp4`)
	Promise.all([unlink(`./UrusMedia/video/${filename}.mp4`), unlink(`./UrusMedia/gif/${filename}.gif`)])
	return buffer5
				   }
...
exports.GIFBufferToVideoBuffer = async (image) => {
    ...
    const filename = `${Math.random().toString(36)}`
    await fs.writeFileSync(`./UrusMedia/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./XeonMedia/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./UrusMedia/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./UrusMedia/trash/${filename}.mp4`)
    Promise.all([unlink(`./UrusMedia/trash/${filename}.mp4`), unlink(`./UrusMedia/trash/${filename}.gif`)])
    return buffer5
}